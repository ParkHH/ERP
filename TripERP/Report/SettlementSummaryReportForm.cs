...
{
    public partial class SettlementSummaryReportForm : Form
    {
        public SettlementSummaryReportForm()
        {
            InitializeComponent();
        }


        private void SettlementSummaryReportForm_Load(object sender, EventArgs e)
        {
            setItemsToSearchArrangementPlaceCompanyComboBox();      // 모객업체 목록 불러오기
        ...
        {
            this.reportViewer1.LocalReport.ReportEmbeddedResource = "TripERP.Report.SettlementSummary.rdlc";      // 보고서와 ReportViewer 연결
        ...
        public DataRow[] getSettlementSateData()
        {
            ...
            string SelectAllForSettlementReport = string.Format("CALL SelectAllForSettlementSummaryReport('{0}','{1}','{2}','{3}')", CMPN_NO, PRDT_CNMB, STMT_FROM_DT, STMT_TO_DT);
        ...
        public void dataRemake(DataRow[] settlementDataArr)
        {
            ...
            for (int i = 0; i < settlementDataArr.Length; i++)
            {
                ...
                string ARPL_CMPN_NM = settlementData["ARPL_CMPN_NM"].ToString().Trim();                                                              // 수배처명
                ...
                double PROFIT = double.Parse(settlementData["PROFIT"].ToString().Trim());                                                                              // GB 수익금
                ...
                dt.Rows.Add(new object[] {
                    ...
                    "￦"+Utils.SetComma(Utils.GetDigitString(WON_PYMT_FEE.ToString())),
                    Utils.SetComma(PROFIT.ToString()),
                    PROFIT_RATE+"%",
                    STMT_DT
                });
            }
            ...
            //------------------------------------------------------------------------------------------------------------------------------------------------------
            var reportParameter = new ReportParameter[12];
            reportParameter[0] = new ReportParameter("SUM_TOT_SALE_WON_AMT", "￦" + Utils.SetComma(SUM_TOT_SALE_WON_AMT));
            reportParameter[1] = new ReportParameter("SUM_STEM_UNPA_WON_AMT", "￦" + Utils.SetComma(SUM_STMT_UNPA_WON_AMT));
            reportParameter[2] = new ReportParameter("SUM_WON_PYMT_FEE", "￦" + Utils.SetComma(Utils.GetDigitString(SUM_WON_PYMT_FEE.ToString())));
            reportParameter[3] = new ReportParameter("SUM_PROFIT", Utils.SetComma(SUM_PROFIT.ToString()));
            reportParameter[4] = new ReportParameter("SUM_PROFIT_RATE", SUM_PROFIT_RATE + "%");
            reportParameter[5] = new ReportParameter("PRINT_DATE", "*출력일자 : " + DateTime.Now.ToString().Trim());    // 출력일자
            reportParameter[6] = new ReportParameter("PrintDoID", "*보고자 : " + setPrintDateAndPrintDoID());                  // 출력자
            ...
            reportParameter[9] = new ReportParameter("SUM_TOT_SALE_FOREIGN_AMT", "$"+Utils.SetComma(SUM_TOT_SALE_FOREIGN_AMT));   // 외화판매금액
            reportParameter[10] = new ReportParameter("SUM_STEM_UNPA_FRCR_AMT", "$"+Utils.SetComma(SUM_STEM_UNPA_FRCR_AMT));       // 현지외화미지급금
            reportParameter[11] = new ReportParameter("CUR_SYBL", CUR_SYBL);
