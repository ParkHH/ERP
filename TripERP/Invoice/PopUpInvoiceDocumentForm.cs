...
{
    public partial class PopUpInvoiceDocumentForm : Form
    {
        private ComboBoxItem cooperationItem = null;                                              // 현재 예약건의 모객업체 정보        
        private ComboBoxItem productItem = null;                                                    // 현재 예약건의 상품정보
        private string DPTR_DT = "";

        public void setCooperationInfo(Object item)
        {
            this.cooperationItem = (ComboBoxItem)item;
        }
        public void setProductInfo(Object item)
        {
            this.productItem = (ComboBoxItem)item;
        }
        public void setDepartureDate(string DPTR_DT)
        {
            this.DPTR_DT = DPTR_DT;
        }
        ...
        public PopUpInvoiceDocumentForm()
        {
            InitializeComponent();
        }
        ...
        //  FORM LOAD 시 동작
        //============================================================================================================================
        private void PopUpInvoiceDocumentForm_Load(object sender, EventArgs e)
        {
            initReportViewerSetting();
        }

        //------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        // REPORT VIEWER SETTING
        //------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        private void initReportViewerSetting()
        {
            ...
            // INVOICE 머리글, 본문 정보 SETTING
            headContentSetting();
            invoiceDocumentContentSetting();

            // INVOICE 수기 입력란 SETTING
            InsertItemForAccount();
            applyData();

            /*
            // INVOICE DATA SOURCE 정보 SETTING
        ...
        //============================================================================================================================ 
        // 본사 정보 가져오기 (Invoice 머리글 정보)          --> 박현호
        //============================================================================================================================
        private void headContentSetting()
        {
        ...
        //============================================================================================================================   
        // INVOICE 내용 정보 가져오기 (Invoice 본문 정보)          --> 박현호
        //============================================================================================================================
        private void invoiceDocumentContentSetting()
        {            
            //-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            ComboBoxItem getCooperationIem = cooperationItem;
            ComboBoxItem getProductItem = productItem;
            string TKTR_CMPN_NO = getCooperationIem.Value.ToString().Trim();
            string PRDT_CNMB = getProductItem.Value.ToString().Trim();
            string DPTR_FROM_DT = DPTR_DT;
            string DPTR_TO_DT = DPTR_DT;
            ...
            var reportParameters = new ReportParameter[4];
            reportParameters[0] = new ReportParameter("SUM_AMT_WON", "￦" + Utils.SetComma(SUM_AMT_WON));                                           // 원화총액
            reportParameters[1] = new ReportParameter("SUM_AMT_FREN", FREN_CUR_SYBL + Utils.SetComma(SUM_AMT_FREN));                      // 외화 총액
            reportParameters[2] = new ReportParameter("CUR_SYBL", FREN_CUR_SYBL.ToString().Trim());                                                           // 통화기호
            reportParameters[3] = new ReportParameter("EXRT_CHANGE", "￦" + Utils.SetComma(FREN_EXRT * SUM_AMT_FREN));                      // 외화환전금액
        ...
        public void applyData()
        {
            ...
            var reportParameters = new ReportParameter[5];
            //------------------------------------------------------------------------------------------------------------
            // 원화계좌은행 Default
            //------------------------------------------------------------------------------------------------------------
            if (bank_won == "" || bank_won.Equals("--- 선택 ----"))
            {
                reportParameters[0] = new ReportParameter("BANK_WON", "국민은행");
            }
            else
            {
                reportParameters[0] = new ReportParameter("BANK_WON", bank_won);
            }
            ...
            if (account_won == "")
            {
                reportParameters[1] = new ReportParameter("ACCOUNT_WON", "763601-04-154658");
            }
            else
            {
                ...
                reportParameters[1] = new ReportParameter("ACCOUNT_WON", account_won);
            }
            ...
            {
                reportParameters[2] = new ReportParameter("BANK_FOREIN", "국민은행");
            }
            else
            {
                reportParameters[2] = new ReportParameter("BANK_FOREIN", bank_foreign);
            }
            ...
            if (account_foreign == "")
            {
                reportParameters[3] = new ReportParameter("ACCOUNT_FOREIN", "763668-11-003753");
            }
            else
            {
                ...
                reportParameters[3] = new ReportParameter("ACCOUNT_FOREIN", account_foreign);
            }
            ...
            {
                reportParameters[4] = new ReportParameter("PAY_END_DATE", DateTime.Now.ToString().Substring(0, 10));
            }
            else
            {
                try
                {
                    ...
                    reportParameters[4] = new ReportParameter("PAY_END_DATE", pay_end_date);
                }
                catch (Exception e)
                {
        ...
        //============================================================================================================================
        // Ctrl + MouseWheel 동작시 보고서 배율 확대      --> 박현호
        //============================================================================================================================
